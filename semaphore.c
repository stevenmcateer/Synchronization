...
semaphore *findChannelLock(int channel);
...
int main(int argc, char **argv) {
    /*   if (argc != 1) {
     ...
     printf("Executing semaphore version of the simulation, press ^C to exit\n");
    ...
    mutex_lock = (semaphore *) malloc(sizeof(semaphore)); // Create a mutex_lock and set it to a default value
    channel_lock1 = (semaphore *) malloc(sizeof(semaphore));//malloc lock for channel 1
    channel_lock6 = (semaphore *) malloc(sizeof(semaphore));//malloc lock for channel 6
    channel_lock11 = (semaphore *) malloc(sizeof(semaphore));//malloc lock for channel 11
    ...
    for (made_nodes = 0; made_nodes < NUM_NODES; made_nodes++) {
        pthread_join(all_nodes[made_nodes].wireless_thread, NULL);
    }
    for (made_noisemakers = 0; made_noisemakers < NUM_NOISEMAKERS; made_noisemakers++) {
 ...
 */
char *idToString(int id) {
    char *stringNum = malloc(sizeof(char) * 25);
    sprintf(stringNum, "%d", id);
    strcat(stringNum, "SemLog.txt");
    return stringNum;
...
void broadcast(wireless_node *node) {
    ...
    if (sem_trywait(findChannelLock(node->channel)) != EAGAIN) {
	...
	if(probRebroadcast >= 50 && node->messagesSeen > 0){
	  ...
	  node->nodeBroadcast = makeBroadcast(node->id, node->channel);
	    ...
	    sem_post(findChannelLock(node->channel));
        } else { ; //do nothing
        }

        if (isBroadcasting) {
            sleep((unsigned) node->trans_time);
	    sem_wait(findChannelLock(channel->node));
